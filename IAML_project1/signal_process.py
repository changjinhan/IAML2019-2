    ...
    if method == 'your_own_way':
        ...
        if first_axis_is_batch:
            ...
            for i in range(len(audio_samples)):
                f.append(np.abs(
                    librosa.cqt(audio_samples[i], sr=float(sr), hop_length=512, bins_per_octave=12, n_bins=7 * 12)))
            cqt = tf.convert_to_tensor(f, dtype=tf.float32)
        else:
            mel_spectrograms = tf.transpose(mel_spectrograms, perm=[1, 0])
            f = np.abs(librosa.cqt(audio_samples, sr=float(sr), hop_length=512, bins_per_octave=12, n_bins=7 * 12))
            cqt = tf.convert_to_tensor(f, dtype=tf.float32)
        return tf.concat([cqt, mel_spectrograms], 0)
